	...
	$("#btnUpdate").click(function(){
		...
		if(confirm("회원정보를 수정하시겠습니까?")){
			...
			//AJAX Fileuplaod
		      ...
		      var formData=new FormData(frm);//binamry전송방식 : 파일업로드 가능
...
if(mDTO == null){
	response.sendRedirect("http://localhost/jsp_prj");//index.html이 자동으로 불러짐 
	return;
}
...
<tr>
<td>성별 </td>
<td><input type="radio" name="gender" value="남자" ${ mDTO.gender eq '남자' ? "checked='checked'":"" }/> </td>
<td><input type="radio" name="gender" value="여자" ${ mDTO.gender eq '여자' ? "checked='checked'":"" }/> </td>
</tr>
<tr>
...
<td><input type="button" id="btnUpdate" value="정보변경" class="btn btn-success"/></td>
<td><fmt:formatDate value="${ mDTO.input_date }" pattern="yyyy-MM-dd a EEEE HH:mm"/></td>
</tr>
</table>
</form>
