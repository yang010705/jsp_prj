	...
	$("#btn").click(function(){
		...
		$.ajax({
			...
			success : function(jsonObj){
				...
				$("#creataData").html(jsonObj.pubData);
				
				$("#deptTab > tbody").empty();
				//데이터
				if(!jsonObj.resultFlag){
					$("#deptTab body").append("<tr><td colspan='3'>부서정보가 존재하지 않습니다.</td></tr>")
				}//end if
				
				$.each(jsonObj.data, function(i, jsonTemp){
					createTr="<tr><td><a href='ajax_emp_json.jsp?deptno=" + jsonTemp.deptno + "'>" + jsonTemp.deptno + "</td><td>" + jsonTemp.dname + "</td><td>" + jsonTemp.loc + "</td></tr>"; 
					$("#deptTab > tbody").append(createTr)
				});
			}
			
		});//ajax
		 var param = $("#deptno").html();
		$.ajax({
			url : "../day0515/json_obj2.jsp",
			type : "POST",
			data: { deptno: param },
			...
			success : function(jsonObj){
				...
				var createTr;
				$.each(jsonObj.data, function(i, jsonTemp){
					createTr="<tr><td>" + (i+1) + "</td><td>" + jsonTemp.empno + "</td><td>" + jsonTemp.ename +
					"</td><td>" + jsonTemp.job + "</td><td>" + jsonTemp.hiredate + "</td><td>" + jsonTemp.sal + "</td></td>";
...
<h2>[<span id="deptno" style="width:50px;  " >]번 부서 사원 정보</span></h2>
